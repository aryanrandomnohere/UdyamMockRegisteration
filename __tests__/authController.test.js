...
const mockValidateOrReturn = validateOrReturn;
    ...
    describe('verifyAadhaar', () => {
        describe('Form Validation Logic', () => {
            test('should return error for invalid Aadhaar length (less than 12 digits)', async () => {
                ...
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'There is error in Aadhaar Validation/Authentication.', expect.arrayContaining(['Aadhaar number must be 12 digits']), 200);
            });
            ...
            test('should return error for empty name', async () => {
                ...
                mockPrisma.udyamRegistration.findMany.mockResolvedValue([]);
                await verifyAadhaar(mockReq, mockRes);
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'There is error in Aadhaar Validation/Authentication.', expect.arrayContaining(['Name is required']), 200);
            });
            test('should return multiple validation errors', async () => {
                ...
                mockPrisma.udyamRegistration.findMany.mockResolvedValue([]);
                await verifyAadhaar(mockReq, mockRes);
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'There is error in Aadhaar Validation/Authentication.', expect.arrayContaining([
        ...
        describe('Database Validation', () => {
            test('should return error when Aadhaar not found in database', async () => {
                ...
                mockPrisma.udyamRegistration.findMany.mockResolvedValue([]);
                await verifyAadhaar(mockReq, mockRes);
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'There is error in Aadhaar Validation/Authentication.', expect.arrayContaining(['There is error in Aadhaar Validation/Authentication.']), 200);
            });
            test('should return error when name does not match', async () => {
                ...
                mockPrisma.udyamRegistration.findMany.mockResolvedValue([]);
                await verifyAadhaar(mockReq, mockRes);
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'There is error in Aadhaar Validation/Authentication.', expect.arrayContaining(['Name is not matching with Aadhaar']), 200);
            });
    ...
    describe('verifyAadhaarOtp', () => {
        describe('Form Validation Logic', () => {
            test('should return error when Aadhaar not found', async () => {
                ...
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'Aadhaar number not found', ['Aadhaar number not found'], 400);
            });
            test('should return error for invalid OTP', async () => {
                ...
                await verifyAadhaarOtp(mockReq, mockRes);
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'Invalid OTP', ['Invalid OTP'], 400);
            });
        });
        describe('Successful Verification', () => {
    ...
    describe('verifyPan', () => {
        describe('Form Validation Logic', () => {
            test('should return error for invalid PAN format', async () => {
                ...
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'Invalid PAN', ['Invalid PAN'], 200);
            });
            test('should validate correct PAN format', async () => {
                ...
                for (const pan of validPanFormats) {
                    ...
                    expect(mockErrorResponse).not.toHaveBeenCalledWith(mockRes, 'Invalid PAN', ['Invalid PAN'], 200);
                    jest.clearAllMocks();
                }
            });
        ...
        describe('Database Validation', () => {
            test('should return error when PAN not found in database', async () => {
                ...
                await verifyPan(mockReq, mockRes);
                expect(mockErrorResponse).toHaveBeenCalledWith(mockRes, 'Details mismatch', ['Details mismatch'], 200);
            });
